...
const API_BASE = APP_CONFIG.backendBaseUrl;
...
async function loadUserInfo(user) {
  try {
    const res = await window.authenticatedFetch(`${API_BASE}/get-user/${user.id}`)
    if (!res.ok) return;
...
async function loadRoadmap(userId) {
  ...
  try {
    const res = await window.authenticatedFetch(`${API_BASE}/get-roadmap/${userId}`);
...
async function loadAchievements(userId) {
  ...
  try {
    const res = await window.authenticatedFetch(`${API_BASE}/get-achievements/${userId}`);
